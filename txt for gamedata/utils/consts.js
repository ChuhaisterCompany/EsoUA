...
const generatePattern = (valueP, ...rest) => {
  const key = `(?<key>${KEY_PATTERN})`;
  const value = `(?<value>${valueP})`;
  const tail = rest.join('');

  return new RegExp(`"${key}"${SEPARATOR_PATTERN}"${value}"${tail}`);
};

const CSV_KEY_PATTERN = generatePattern(VALUE_PATTERN);
const CSV_PATTERN_FOR_UPDATE = generatePattern(VALUE_PATTERN_LAZY, `${SEPARATOR_PATTERN}""`);
